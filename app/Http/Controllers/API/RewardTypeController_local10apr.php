    ...
    {   
        if(auth('sanctum')->check()){
            ...
            if($rewardType->count() > 0){
                $rewardtypeCount = $rewardType->count();
                for($i=0;$i<$rewardtypeCount;$i++){
                    // print_r($rewardType[$i]->id);
                    $rewardUser = RewardPoint::where('user_id', $userId)->where('reward_type_id', $rewardType[$i]->id)->whereDate('created_at', '>=', $todatStartDateTime)->whereDate('created_at', '<=', $todatEndDateTime)->first();
                }
                if(empty($rewardUser)){
                    echo "empty";
                }else{
